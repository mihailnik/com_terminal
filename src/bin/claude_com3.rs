...
impl ComTerminal {
    ...
    fn subscription(&self) -> Subscription<Message> {
        if self.port_settings.connected {
            let port_arc = self.serial_port.clone().unwrap();

            let read_stream = futures::stream::unfold(port_arc, |port| async move {
                let message = {
                    // Создаём новый блок
                    ...
                    match port_lock.read(&mut buffer) {
                        Ok(bytes_read) => {
                            if bytes_read > 0 {
                                ...
                                Some(Message::DataReceived(data))
                            } else {
                                Some(Message::DataReceived("".to_string()))
                            }
                        }
                        Err(ref e) if e.kind() == io::ErrorKind::TimedOut => {
                            // Timeout: return None from this inner block
                            None
                        }
                        Err(e) => Some(Message::PortError(e.to_string())),
                    }
                }; // port_lock выходит из области видимости и уничтожается
            ...
            return Subscription::run_with_id("com_terminal_read_stream", read_stream);
        }
        Subscription::none()
    }
